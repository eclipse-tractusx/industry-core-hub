/********************************************************************************
 * Eclipse Tractus-X - Industry Core Hub Frontend
 *
 * Copyright (c) 2026 Contributors to the Eclipse Foundation
 *
 * See the NOTICE file(s) distributed with this work for additional
 * information regarding copyright ownership.
 *
 * This program and the accompanying materials are made available under the
 * terms of the Apache License, Version 2.0 which is available at
 * https://www.apache.org/licenses/LICENSE-2.0.
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND,
 * either express or implied. See the
 * License for the specific language governing permissions and limitations
 * under the License.
 *
 * SPDX-License-Identifier: Apache-2.0
 ********************************************************************************/

import { ReactNode } from 'react';
import { Button, ButtonProps } from '@mui/material';
import { kitThemes } from '@/theme/colors';

interface PrimaryActionButtonProps extends Omit<ButtonProps, 'variant' | 'sx'> {
  startIcon?: ReactNode;
  onClick: () => void;
  disabled?: boolean;
  children: ReactNode;
}

/**
 * CCM KIT primary action button — purple gradient consistent with the Upload
 * Certificate button in Certificate Management.
 */
const PrimaryActionButton = ({
  startIcon,
  onClick,
  disabled = false,
  children,
  ...rest
}: PrimaryActionButtonProps) => (
  <Button
    variant="contained"
    startIcon={startIcon}
    onClick={onClick}
    disabled={disabled}
    {...rest}
    sx={{
      background: `linear-gradient(135deg, ${kitThemes.ccm.gradientStart} 0%, ${kitThemes.ccm.gradientEnd} 100%)`,
      color: '#fff',
      borderRadius: { xs: '10px', md: '12px' },
      fontWeight: 600,
      textTransform: 'none',
      boxShadow: `0 4px 16px ${kitThemes.ccm.shadowColor}`,
      transition: 'all 0.2s ease',
      '&:hover': {
        filter: 'brightness(1.1)',
        boxShadow: `0 6px 24px ${kitThemes.ccm.shadowColor}`,
        transform: 'translateY(-1px)',
        background: `linear-gradient(135deg, ${kitThemes.ccm.gradientStart} 0%, ${kitThemes.ccm.gradientEnd} 100%)`,
      },
      '&.Mui-disabled': {
        background: 'rgba(255,255,255,0.12)',
        color: 'rgba(255,255,255,0.3)',
        boxShadow: 'none',
      },
    }}
  >
    {children}
  </Button>
);

export default PrimaryActionButton;
